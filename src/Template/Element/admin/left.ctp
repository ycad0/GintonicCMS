    ...
    <section class="sidebar">
        <div class="user-panel">
            <div class="pull-left image">
                <?php
                echo $this->Html->image($this->File->getFileUrl($this->request->session()->read('Auth.User.file.filename')),['class' => 'img-circle']);
                ?>
            </div>
            <div class="pull-left info">
                <p>
                    <?php echo $this->Html->link($this->request->session()->read('Auth.User.first') . ' ' . $this->request->session()->read('Auth.User.last'),['controller'=>'users','action'=>'profile']);?>
                </p>
                <span title="<?php echo $this->request->session()->read('Auth.User.email');?>"><?php echo $this->request->session()->read('Auth.User.email');?></span>
            </div>
        </div>        
